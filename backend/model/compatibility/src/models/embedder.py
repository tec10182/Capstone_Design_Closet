...
from model.compatibility.src.datasets.processor import FashionInputProcessor
...
class OutfitTransformerEmbeddingModel(nn.Module):
    def __init__(
        ...
        self.image_encoder.fc = nn.Linear(
            self.image_encoder.fc.in_features, self.encoder_hidden, bias=False
        )

        # Text Encoder
        self.text_encoder = AutoModel.from_pretrained(huggingface)
        self.text_projection = nn.Parameter(
            torch.empty(self.text_encoder.config.hidden_size, self.encoder_hidden)
        )
        freeze_model(self.text_encoder)
        nn.init.normal_(
            self.text_projection, std=self.text_encoder.config.hidden_size**-0.5
        )
    ...
    def encode(self, inputs):
        ...
        if "input_ids" in inputs:
            ...
            text_embeds = (
                mean_pooling(text_outputs, inputs["attention_mask"])
        ...
        return {
            "mask": inputs["mask"] if inputs.get("mask") is not None else None,
            "embeds": embeds,
        }
