...
if __name__ == "__main__":
    ...
    if st.button('입어보기!'):
        data = {
            "p_img_name": st.session_state['person_img_name'],
            "c_img_name": st.session_state['cloth_img_name']
        }
        res = requests.post(
            url=f"http://127.0.0.1:8000/api/v1/tryon/generate" ,data=data)

        img_name = res.text.replace('"', '').replace("\\", "")

        st.image(osp.join('D:\\storage\\tryon\\output', img_name))